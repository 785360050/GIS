...
void Animate_Symbol::_Signal_Bind()
{
    ...
    connect(timer, &QTimer::timeout, this, [&]()
    {
        ...
        qDebug()<<frame_index;
        setMissionFrame(frame_index+1);
        if(frame_index>=missionSize())
            setMissionFrame(0);
    });
...
void Animate_Symbol::changeMission(const QString &missionNameStr)
{
    ...
    mission_data.parse(DATA_PATH + "/Missions/" + formattedname.remove(" ") + ".csv");

    // if the mission was loaded successfully, move to the start position
    if (missionReady() && graphic_route)
    {
        ...
        {
            const Mission_Data::Point& dp = mission_data[i];
...
void Animate_Symbol::animate()
{
    ...
    if (missionFrame() < missionSize())
    {
        ...
        const Mission_Data::Point& dp = mission_data[missionFrame()];

        // move 3D graphic to the new position
        graphic_3d->setGeometry(dp.m_pos);
        // update attribute expressions to immediately update rotation
        graphic_3d->attributes()->replaceAttribute(HEADING, dp.m_heading);
        graphic_3d->attributes()->replaceAttribute(PITCH, dp.m_pitch);
        graphic_3d->attributes()->replaceAttribute(ROLL, dp.m_roll);

        // move 2D graphic to the new position
        graphic_2d->setGeometry(dp.m_pos);
        symbol_2d->setAngle(dp.m_heading);
    }
...
void Animate_Symbol::createModel2d(GraphicsOverlay *mapOverlay)
{
    ...
    graphic_2d = new Graphic(dp.m_pos, symbol_2d, this);
...
void Animate_Symbol::createGraphic3D()
{
    ...
    if (!graphic_3d)
    {
        ...
        graphic_3d = new Graphic(dp.m_pos, model_3d, this);
        graphic_3d->attributes()->insertAttribute(HEADING, dp.m_heading);
        graphic_3d->attributes()->insertAttribute(PITCH, dp.m_pitch);
        graphic_3d->attributes()->insertAttribute(ROLL, dp.m_roll);
